...
def load_ifa(location_ids):
    ...
    ifa = ifa.loc[(ifa.location_id.isin(location_ids)) & (ifa.year_id == 2023)]
...
def load_anc_art(art, location):
    anc = (
        art.load("covariate.antenatal_care_1_visit_coverage_proportion.estimate")
        .rename_axis("draw", axis=1)
        .stack()
        .rename("value")
        .reset_index()
    )
...
def load_art_exposure():
    art_exposure = pd.read_csv("lbwsg_category_data_gbd_2021.csv").rename(
        columns={"lbwsg_category": "parameter"}
    )
    return art_exposure[
        ["parameter", "ga_start", "ga_end", "bw_start", "bw_end"]
    ]  # Only keep the columns that we need right now


def initialize_simulation(location, draw):
    no_oral_iron_spec = build_model_specification(
        Path("../../model_specifications/model_spec.yaml")
    )
    ...
    no_oral_iron_spec.components.vivarium_gates_mncnh.components = [
        c
        for c in no_oral_iron_spec.components.vivarium_gates_mncnh.components
        if "OralIron" not in c
        and "iron_folic_acid" not in c
        and "multiple_micronutrient_supplementation" not in c
    ]
    # specify draw number
    no_oral_iron_spec.configuration.input_data.input_draw_number = draw
    # specify artifact path/location
    no_oral_iron_spec.configuration.input_data.artifact_path = Path(ART_DIR) / (
        location + ".hdf"
    )
...
def lookup_continuous_bw_ga(row, art_exposure):
    for (param, ga_start, ga_end, bw_start, bw_end) in art_exposure.itertuples(
...
def return_sim_data(location, draw, art_exposure):
    ...
    df = (
        df.groupby("anc1")
        .parameter.value_counts(normalize=True)
        .rename("exposure")
        .reset_index()
    )
    # check expsure values sum to 1
    assert np.all(
        df.groupby("anc1").exposure.sum() == 1
    ), "LBWSG exposure values across categories do not sum to 1"
    # now check to see that we have all expected categories represented in our data
    if (
        len([x for x in art_exposure.parameter.unique() if x not in df.parameter.unique()])
        > 0
    ):
        print(
            f"WARNING: not all LBWSG exposure categories represented in interactive simulation population for draw {draw} in {location}. Consider increasing population size and rerunning"
        )
...
def calculate_rrs_from_shifts(shifts):
    ...
    exp_uncovered_no_anc["frac_ptb"] = np.where(
        exp_uncovered_no_anc.ga_start >= 37,
        0,
        np.where(
            exp_uncovered_no_anc.ga_end < 37,
            1,
            (37 - exp_uncovered_no_anc.ga_start)
            / (exp_uncovered_no_anc.ga_end - exp_uncovered_no_anc.ga_start),
        ),
    )
    uncovered_no_anc_ptb = (
        exp_uncovered_no_anc.reset_index().exposure
        * exp_uncovered_no_anc.reset_index().frac_ptb
    ).sum()

    exp_covered_no_anc = exp_uncovered_no_anc + shift_false
    exp_covered_no_anc["frac_ptb"] = np.where(
        exp_covered_no_anc.ga_start >= 37,
        0,
        np.where(
            exp_covered_no_anc.ga_end < 37,
            1,
            (37 - exp_covered_no_anc.ga_start)
            / (exp_covered_no_anc.ga_end - exp_covered_no_anc.ga_start),
        ),
    )
    covered_no_anc_ptb = (
        exp_covered_no_anc.reset_index().exposure * exp_covered_no_anc.reset_index().frac_ptb
    ).sum()

    exp_uncovered_anc = anc - weighted_avg_shift * coverage
    exp_uncovered_anc["frac_ptb"] = np.where(
        exp_uncovered_anc.ga_start >= 37,
        0,
        np.where(
            exp_uncovered_anc.ga_end < 37,
            1,
            (37 - exp_uncovered_anc.ga_start)
            / (exp_uncovered_anc.ga_end - exp_uncovered_anc.ga_start),
        ),
    )
    uncovered_anc_ptb = (
        exp_uncovered_anc.reset_index().exposure * exp_uncovered_anc.reset_index().frac_ptb
    ).sum()

    exp_covered_anc = exp_uncovered_anc + shift_true
    exp_covered_anc["frac_ptb"] = np.where(
        exp_covered_anc.ga_start >= 37,
        0,
        np.where(
            exp_covered_anc.ga_end < 37,
            1,
            (37 - exp_covered_anc.ga_start)
            / (exp_covered_anc.ga_end - exp_covered_anc.ga_start),
        ),
    )
    covered_anc_ptb = (
        exp_covered_anc.reset_index().exposure * exp_covered_anc.reset_index().frac_ptb
    ).sum()
...
def generate_location_draw_global_data(
    location, draw, anc_art, baseline_ifa, art_exposure, ifa_rr_draws
):
    ...
    data = data.merge(
        baseline_ifa.rename(columns={"value": "ifa_coverage"}), on=["location_id", "draw"]
    )
    ...
    anc = exp[exp["anc1"] == True].set_index(
        [c for c in data.columns if "ga_" not in c]
    )  # anc from interactive sim - gestational age columns are the operative ones (shift gets added to these)
...
def perform_single_optimization():
    ...
    return scipy.optimize.minimize(
        shift_optimization, [0, 0], tol=10e-4, method="Nelder-Mead"
    )
...
def store_optimization_results(
    ...
    coverage, anc, no_anc, anc_coverage, rr = generate_location_draw_global_data(
        location, draw, anc_art, baseline_ifa, art_exposure, ifa_rr_draws
    )
...
def run_location_draws(location, draws, args_all_locations):
    ...
    baseline_ifa = load_baseline_ifa(
        anc_art, ifa[ifa["location_id"] == LOCATION_DATA[location]]
    )
    results = pd.DataFrame()
    anc_exp = pd.DataFrame()

    for draw in draws:
        output, exp_res = store_optimization_results(
            location, draw, anc_art, baseline_ifa, art_exposure, ifa_rr_draws
        )
...
def load_mms_data(exp_anc, ifa_shifts):
    anc_art = pd.concat(
        [
            load_anc_art(load_artifact(location), location)
            for location in list(LOCATION_DATA.keys())
        ]
    )
    ifa = load_ifa(list(LOCATION_DATA.values()))
    ...
    exp_prepped = (
        exp_anc.set_index([c for c in exp_anc.columns if "draw" not in c])
        .stack()
        .reset_index()
    )
    ...
    shift_true = (
        ifa_shifts.rename(columns={"shift_anc": "value"})
        .drop("shift_no_anc", axis=1)
        .set_index(["location_id", "draw"])
    )
    shift_false = (
        ifa_shifts.rename(columns={"shift_no_anc": "value"})
        .drop("shift_anc", axis=1)
        .set_index(["location_id", "draw"])
    )
    weighted_avg_shift = (shift_true * anc_art + shift_false * (1 - anc_art)).dropna()
    covered_shift = (
        -weighted_avg_shift * baseline_ifa.set_index(["location_id", "draw"]) + shift_true
    )
    covered_shift = (
        covered_shift.reset_index().rename(columns={"value": "covered_shift"}).dropna()
    )
    ...
    covered_exp["frac_ptb"] = np.where(
        covered_exp.ga_start > 37,
        0,
        np.where(
            covered_exp.ga_end < 37,
            1,
    ...
    covered_exp["frac_vptb"] = np.where(
        covered_exp.ga_start >= 32,
        0,
        np.where(
            covered_exp.ga_end < 32,
            1,
    ...
    covered_ptb = covered_exp.groupby(["location_id", "draw"]).sum()[
        ["exposure_ptb", "exposure_vptb"]
    ]
    covered_ptb.groupby("location_id").describe(percentiles=[0.025, 0.975])[["exposure_ptb"]]

    mms_pt_dist = lognorm_from_median_lower_upper(
        0.91, 0.84, 0.99, quantile_ranks=(0.025, 0.975)
    )
    ...
    mms_vpt_dist = lognorm_from_median_lower_upper(
        0.81, 0.71, 0.93, quantile_ranks=(0.025, 0.975)
    )
    ...
    data_mms_pt = (
        ifa_exp.drop("rr", axis=1)
        .merge(mms_pt_rr_draws, on="draw")
        .drop(columns=["frac_vptb"])
    )
    data_mms_vpt = (
        ifa_exp.drop("rr", axis=1)
        .merge(mms_vpt_rr_draws, on="draw")
        .drop(columns=["frac_ptb"])
    )
...
def mms_shift(draw, location_id, data_mms_pt, data_mms_vpt, ptb=True):
    ...
    exp_uncovered = data.loc[
        (data.draw == f"draw_{draw}") & (data.location_id == location_id)
    ]
    uncovered_low = exp_uncovered[f"exposure_{threshold}"].sum()

    def shift_optimization(shift):
        exp_covered = (
            exp_uncovered.set_index([c for c in exp_uncovered.columns if "ga_" not in c])
            + shift
        )
        exp_covered = exp_covered.reset_index()
        exp_covered[f"frac_{threshold}"] = np.where(
            exp_covered.ga_start >= threshold_val,
            0,
            np.where(
                exp_covered.ga_end < threshold_val,
                1,
                (threshold_val - exp_covered.ga_start)
                / (exp_covered.ga_end - exp_covered.ga_start),
            ),
        )
    ...
    return scipy.optimize.minimize_scalar(
        shift_optimization, bounds=(-2, 2), method="bounded"
    )["x"]
...
def mms_double_shift(draw, location_id, data_mms_vpt, mms_pt_rr_draws, mms_vpt_rr_draws):
    ...
    exp_uncovered = data_mms_vpt.loc[
        (data_mms_vpt.draw == f"draw_{draw}") & (data_mms_vpt.location_id == location_id)
    ]
    ...
    def vptb_shift_optimization(shift):
        ...
        exp_covered = exp_covered.reset_index()
        exp_covered["frac_vptb"] = np.where(
            exp_covered.ga_start >= 32,
            0,
            np.where(
                exp_covered.ga_end < 32,
                1,
    ...
    vptb_shift = scipy.optimize.minimize_scalar(
        vptb_shift_optimization, bounds=(-2, 2), method="bounded"
    )["x"]
    mms1 = (
        exp_uncovered.set_index([c for c in exp_uncovered.columns if "ga_" not in c])
        + vptb_shift
    ...
    mms1["frac_vptb"] = np.where(
        mms1.ga_start >= 32,
        0,
        np.where(mms1.ga_end < 32, 1, (32 - mms1.ga_start) / (mms1.ga_end - mms1.ga_start)),
    )
    mms1["frac_ptb"] = np.where(
        mms1.ga_start >= 37,
        0,
        np.where(mms1.ga_end < 37, 1, (37 - mms1.ga_start) / (mms1.ga_end - mms1.ga_start)),
    )

    def second_shift_optimization(shift):
        mms2 = mms1.copy()
        mms2["ga_start"] = np.where(
            mms2.ga_start < (32 - shift), mms2.ga_start, mms2.ga_start + shift
        )
        mms2["ga_end"] = np.where(
            mms2.ga_start < (32 - shift), mms2.ga_end, mms2.ga_end + shift
        )
        mms2 = mms2.reset_index()
        mms2["frac_ptb"] = np.where(
            mms2.ga_start >= 37,
            0,
            np.where(
                mms2.ga_end < 37, 1, (37 - mms2.ga_start) / (mms2.ga_end - mms2.ga_start)
            ),
        )
        mms2["frac_vptb"] = np.where(
            mms2.ga_start >= 32,
            0,
            np.where(
                mms2.ga_end < 32, 1, (32 - mms2.ga_start) / (mms2.ga_end - mms2.ga_start)
            ),
        )
    ...
    second_shift = scipy.optimize.minimize_scalar(
        second_shift_optimization, bounds=(-5, 5), method="bounded"
    )["x"]

    return vptb_shift, second_shift


def mms_run_all_locations(
    draws_to_run, data_mms_pt, data_mms_vpt, mms_pt_rr_draws, mms_vpt_rr_draws
):
    results = []
    for location_id in list(LOCATION_DATA.values()):
        for draw in draws_to_run:
            vptb_shift, second_shift = mms_double_shift(
                draw, location_id, data_mms_vpt, mms_pt_rr_draws, mms_vpt_rr_draws
            )
            results.append(
                {
