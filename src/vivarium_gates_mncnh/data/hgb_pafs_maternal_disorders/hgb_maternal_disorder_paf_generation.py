...
def load_rrs(location, draw, population_size, md_rr_data):
    ...
    pop["age_group"] = pd.cut(pop["age"], bins=bins, labels=labels, right=False, include_lowest=True)
...
def calculate_pafs(location, draw, population_size, md_rr_data):
    ...
    md_mean_rr = data.drop(columns=['age','hemoglobin_exposure','pregnancy_outcome']).groupby(['location','age_group'])[[
        x for x in data.columns if 'neonatal_sepsis' not in x and 'rr' in x]].mean()
