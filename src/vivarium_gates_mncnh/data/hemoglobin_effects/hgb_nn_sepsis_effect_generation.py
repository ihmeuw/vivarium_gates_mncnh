...
def get_simulated_population(location, draw):
    ...
    path = Path('/mnt/share/homes/alibow/vivarium_gates_mncnh/src/vivarium_gates_mncnh/model_specifications/model_spec.yaml')
    custom_model_specification = build_model_specification(path)
    del custom_model_specification.configuration.observers
    custom_model_specification.configuration.input_data.artifact_path = artifact_directory + location + ".hdf"
    ...
    # WARNING: this needs to be manually maintained
...
def load_interpolators(location, draw):
    art = Artifact(artifact_directory + location + '.hdf', filter_terms={'child_age_end < 0.5'})
    interpolators = art.load('risk_factor.low_birth_weight_and_short_gestation.relative_risk_interpolator')[f'draw_{draw}'].reset_index()
...
def calculate_indirect_effect(location, draw):
    ...
    shifts = shifts.pivot_table(index=['sex_of_child','exposure'],
                                values='value',
                                columns='outcome').reset_index()
    ...
    for exposure_level in exposure_levels:
        ...
        enn_log_relative_risk[is_male] = (interpolators['Male', 2](gestational_age[is_male],
                                                            birth_weight[is_male],
                                                            grid=False))
        enn_log_relative_risk[~is_male] = (interpolators['Female', 2](gestational_age[~is_male],
                                                                birth_weight[~is_male],
                                                                grid=False))
        lnn_log_relative_risk[is_male] = (interpolators['Male', 3](gestational_age[is_male],
                                                            birth_weight[is_male],
                                                            grid=False))
        lnn_log_relative_risk[~is_male] = (interpolators['Female',3](gestational_age[~is_male],
                                                                birth_weight[~is_male],
                                                                grid=False))
        enn_relative_risk = np.exp(enn_log_relative_risk)
        lnn_relative_risk = np.exp(lnn_log_relative_risk)
        sub_data['enn_indirect_effect'] = enn_relative_risk / sub_data.effect_of_low_birth_weight_and_short_gestation_on_early_neonatal_neonatal_sepsis_and_other_neonatal_infections_relative_risk
        sub_data['lnn_indirect_effect'] = lnn_relative_risk / sub_data.effect_of_low_birth_weight_and_short_gestation_on_late_neonatal_neonatal_sepsis_and_other_neonatal_infections_relative_risk
        enn_indirect_effect = sub_data.loc[sub_data.enn_pop].groupby('sex_of_child').enn_indirect_effect.mean().reset_index().rename(columns={'enn_indirect_effect': 'value'})
        enn_indirect_effect['age_group_id'] = 2
        lnn_indirect_effect = sub_data.loc[sub_data.lnn_pop].groupby('sex_of_child').lnn_indirect_effect.mean().reset_index().rename(columns={'lnn_indirect_effect': 'value'})
