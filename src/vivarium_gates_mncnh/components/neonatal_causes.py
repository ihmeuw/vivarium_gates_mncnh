...
class NeonatalCause(Component):
    ...
    def get_paf(self, builder: Builder) -> str:
        return PIPELINES.LBWSG_ACMR_PAF_MODIFIER
    ...
    def get_normalized_csmr(self, index: pd.Index) -> pd.Series:
        ...
        normalizing_constant = 1 - self.population_view.get(index, self.lbwsg_acmr_paf)
    ...
    def modify_death_in_age_group_probability(
        ...
        intermediate_csmr = self.population_view.get(index, self.intermediate_csmr_name)
        final_csmr = self.population_view.get(index, self.final_csmr_name)
        # ACMR = ACMR - CSMR + CSMR
...
class PretermBirth(NeonatalCause):
    ...
    def get_normalized_csmr(self, index: pd.Index) -> pd.Series:
        pop = self.population_view.get(index, [COLUMNS.GESTATIONAL_AGE_EXPOSURE])
